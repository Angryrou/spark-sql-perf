...
object MyRunSimu {
  ...
  def main(args: Array[String]): Unit = {
    val parser = new scopt.OptionParser[RunSimuConfig]("Run-Simu") {
    ...
    parser.parse(args, RunSimuConfig()) match {
      case Some(config) =>
        run(config)
      case None =>
        System.exit(1)
  ...
  def run(config: RunSimuConfig): Unit = {
